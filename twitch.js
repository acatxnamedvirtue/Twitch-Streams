...
function selectButton(e) {
  ...
  if(e.currentTarget.id === "all") {
    $('li').removeClass("hide");
  } else if(e.currentTarget.id === "online") {
    $('li.online').removeClass("hide");
    $('li.offline').addClass("hide");
  } else {
    $('li.online').addClass("hide");
    $('li.offline').removeClass("hide");
  }
}

function fillStreamList() {
  _.forEach(streams, function(str) {
    if(str.stream === null) {
      var html = "<li class='clearfix stream offline'><img src=https://dummyimage.com/50x50/ecf0e7/5c5457.jpg&text=0x3F class='logo'><div class='account'><a href='https://www.twitch.tv/" + str.display_name + "'>" + str.display_name + "</a></div><div class='desc'>Offline</div></li>"
      $('ul').append(html);
    } else if(str.error === "Not Found") {
      var html = "<li class='clearfix stream offline'><img src=https://dummyimage.com/50x50/ecf0e7/5c5457.jpg&text=0x3F class='logo'><div class='account'>Account Closed</div><div class='desc'>Account Closed</div></li>"
      $('ul').append(html);
    } else {
      var html = "<li class='clearfix stream online'><img src=" + str.stream.logo + " class='logo'><div class='account'><a href='https://www.twitch.tv/" + str.stream.display_name + "'>" + str.stream.display_name + "</a></div><div class='desc'>" + str.stream.game + ": " + str.stream.status + "</div></li>";
      $('ul').append(html);
    }
  });
}
